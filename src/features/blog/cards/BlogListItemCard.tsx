...
export default function BlogListItemCard({
  data,
  imagePosition = "left",
}: Props) {
  return (
    <Link href={`/blog/${data.slug}`} className="hover:bg-white/20 hover:rounded-lg ">
      <div
        className={`flex gap-2 h-17 lg:h-29 ${imagePosition === "right" ? "flex-row-reverse" : "flex-row"}`}
      >
        <img
          src={data.thumbnail}
          alt={data.title}
