      ...
      <div className="flex items-center gap-3">
        <Link href="/user/orders">
          <Button
            ...
            variant="dark"
            padding="rounded"
            className="rounded-xl bg-(--bg-neutral-secondary-medium) border border-(--border-default-medium) flex-shrink-0"
          />
        </Link>

        <div className="flex items-center gap-1.5 min-w-0">
          <span className="text-sm sm:text-[16px] font-semibold text-(--text-white) truncate">
          ...
          <Button
            ...
            size="xs"
            padding="rounded"
            className="w-[22px] h-[22px] p-1 rounded-full border bg-(--bg-neutral-secondary-medium) text-(--text-body) border-(--border-default-medium) transition-colors flex-shrink-0"
            title={copied ? "Kopyalandı!" : "Kopyala"}
          />
        </div>
      </div>

      {/* Sipariş Durumu + Toplam */}
