    ...
    <div className="w-full max-w-96 mx-auto animate-in fade-in duration-500">
      ...
      {!onSuccess && (
        <div className="mb-8">
          ...
          <p className="text-(--text-body) text-sm mb-2">{t("subtitle")}</p>
          <div className="flex gap-1 text-sm">
            <span className="text-(--text-body)">{t("hasAccount")}</span>
            <button
              onClick={() => router.push("/login")}
              className="text-(--text-fg-brand) font-medium hover:underline transition-all"
            >
              {tBtn("login")}
            </button>
          </div>
      ...
      >
          ...
          <div className="space-y-3 animate-in fade-in slide-in-from-top-1">
            ...
            <div className="space-y-2">
              ...
              <ul className="grid grid-cols-1 gap-1.5">
                ...
                <ValidationItem
                  label={tPw("hasNumber")}
                  isValid={validationRules.hasNumber}
                />
                <ValidationItem
                  label={tPw("hasCase")}
                  isValid={
                    validationRules.hasUpperCase && validationRules.hasLowerCase
                  }
                />
                <ValidationItem
                  label={tPw("hasSymbol")}
                  isValid={validationRules.hasSymbol}
                />
              </ul>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Input
            ...
            aria-invalid={
              formData.passwordAgain.length > 0
...
function ValidationItem({
  ...
  return (
    <li
      className={`flex items-center gap-2 text-[11px] transition-all ${isValid ? "text-(--text-fg-success)" : "text-gray-400"}`}
    >
      {isValid ? (
        <Check className="w-3.5 h-3.5" />
      ) : (
        <Close className="w-3.5 h-3.5 opacity-50" />
      )}
      <span>{label}</span>
    </li>
  );
