        ...
        {mockNavLinkCards.map((navLink) => (
          <Link key={navLink.id} href={`/${navLink.slug}`} className="block group">
            <BaseCard
              ...
              className={`relative w-31.5 h-18 my-3 rounded-lg overflow-hidden flex text-center ${navLink.isPremium ? 'items-center justify-center border-none' : 'items-s tart justify-start'
                }`}
            >
              ...
              {/* Title */}
              <span className={`relative z-10 font-inter text-sm font-semibold ${navLink.isPremium ? 'text-[#FDC700]' : 'text-white p-2'
                }`}>
                {navLink.title}
              </span>
            </BaseCard>
